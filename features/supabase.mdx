---
title: "Connect Supabase to native.builder"
sidebarTitle: "Supabase"
description: "Connect Supabase to your workspace, link each app to a project, toggle agent access, store secrets, and fix common auth issues."
keywords: ["supabase", "database", "auth", "postgres", "storage", "connect supabase"]
"og:title": "Connect Supabase to native.builder"
"og:description": "Connect Supabase to your workspace, link each app to a project, toggle agent access, store secrets, and fix common auth issues."
---
...
You also need to be an **owner or admin** of your <span class="brand-name">native.builder</span> workspace to connect Supabase. Members can use a connection that’s already set up, but can’t create or remove one. If the **Connect** button isn’t there, ask whoever owns your workspace.
...
## Speechmatics and other API keys

Apps that call third-party APIs (for example [Speechmatics](/features/speechmatics) speech-to-text) store secrets through Supabase Edge Functions so keys never ship in the browser. Connect Supabase first, then ask Builder to add the integration.
