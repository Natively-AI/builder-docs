---
title: "Workspaces in native.builder"
sidebarTitle: "Workspaces"
description: "Run an internal AI software factory with native.builder workspaces — members, roles, shared credits, and integrations."
keywords: ["workspace", "team", "invite members", "collaboration", "roles", "ai software factory"]
"og:title": "Workspaces in native.builder"
"og:description": "Run an internal AI software factory with native.builder workspaces — members, roles, shared credits, and integrations."
---
...
## Share a single project by link

Inviting a teammate (above) makes them a member of the whole workspace. If you only want to give someone access to **one project** — without adding them as a workspace member — share that project by link instead.
...
<Steps>
  ...
  <Step title="Get the link">
    Under **Share via link**, wait for **Generating link…** to finish, then click **Copy link**.
  </Step>
  <Step title="Send it">
    Send the link to whoever you want to have access. They don't need to be a workspace member to use it.
  </Step>
</Steps>

<Note>
  This is separate from **Settings → Members**. A share link grants access to that one project only; inviting someone as a member (above) gives them a role across the whole workspace.
</Note>
